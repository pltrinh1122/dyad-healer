...
- [ ] ⏸ **Practice governance discipline** — codify a small set of anchor-maintenance + quality-standard rules surfaced this morning across multiple FEEDBACK/IDEATE cycles. **Sub-items:** **(a) anchor-sync** — every ratified new `kb/` doc updates `CLAUDE.md`'s Durable practice map in the same ratification cycle (closes anchor-divergence gap); **(b) Practice lifecycle** — archival convention (`ledger/archive/YYYY-QN.md` > 2 quarters; `ledger/backlog-archive.md` Done > 1 quarter) + new `kb/practice-snapshot.md` (RATIFIED-state digest) + CLAUDE.md anchor points to snapshot + active loop, not full historical ledger; **(c) kb-doc quality standard** — every kb/ doc with a self-extension or Falsifiability section must carry *proactive orthogonality criteria* (pattern recognized after 2 FEEDBACK cycles 2026-05-28: seizure-taxonomy P4-refinement + glossary refinement — same shape both times); **(d) glossary-check at ratification** — new cross-cutting tokens invoked 3+ times across kb/ require a glossary entry in the same ratification cycle; **(e) form-alignment audit** — per-ratification check against the parent Dyad-Practice form's policy on terms we use (closes the drift-via-bundled-ratification gap that surfaced as the *Dao* retirement); **(f) resume-mode `git-status` habit** — always `git status` + `git diff HEAD` before mid-edit operations across session boundaries (the parallel-fork near-clobber lesson). Trigger criteria for (b): ledger.md > 1000 lines OR backlog.md Done > 50 items OR verify-before-asserting canary trips on practice-loading. (a)/(c)/(d)/(e)/(f) bind on every ratification cycle once codified. **Queued post-Loop-#4.** Provenance: 2026-05-28 multiple [FEEDBACK]/[IDEATE] cycles; see ledger §"SPAOR phase-enrichment proposal" + the reflections this section's sub-items emerged from.
### Done
