...
### Open
- [ ] **Lexical-guard false-positive cascade — DRAFTED + RATIFIED, pending filing** 2026-05-29. From case-04 S9 carry ("guard bug + harmonization deferred"). One filing, two violations, ailment-only: **V1** guard precision/scope (`tests/test_lexical_guard.py` `check_content()` raw substring + scans tracked `.venv` 670 files); **V2** deferred-debt locality (`dao/ziran/align` recorded in retro + *proposed* ledger note, absent from active `terms:`). Harmonized SG-0003/SG-0002 (V1) · SG-0005/NS-0001 (V2). Re-verified live at `dz-cil@8507664`. A (`#1393`-V2 path) dropped — already filed. → `recommendations/2026-05-29-lexical-guard-false-positive-cascade.md`. **Next:** Healer files via Requirement Intake template on Operator trigger.
### Done
- [x] **Agent-ROM integrity violations — FILED** 2026-05-29 → [dz-cil#1393](https://github.com/pltrinh1122/dz-cil/issues/1393) (OPEN, `status:triage`). Two violations as filed: **V1** `AGENT.md` Rule 7 (formerly §6.7) ↔ WIP-N=1 HTIL contradiction (exception promoted to `GEMINI.md` only, not the universal doc); **V2** `GEMINI.md:10` broken `./skills/audit_daemon.py` bring-up reference (actual `drivers/audit_daemon.py`). Harmonized SG-0002/SG-0003/NS-0001; x-ref #1233. Filed **ailment-only** (no prescribed cure) per Operator [FEEDBACK] + describe-the-wound discipline. **Third originally-flagged defect (`triage` vs `status:triage` label) dropped — patient already self-corrected** (verified at time-of-use). Source: backlog Pillar-2 open item → `recommendations/2026-05-29-agent-rom-integrity-violations.md`.
