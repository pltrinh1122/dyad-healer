...
- [ ] **A · Prescriptions-as-guards (CSI frontier step A — LEANED FIRST).** Shift the prescription form from prose →
  ...
  **First instance DRAFTED 2026-06-06** → `recommendations/2026-06-06-discard-invariant-guard.md` — re-derived live
  (wound at `daemon_node.py:78`; patient's Discard Invariant is prose-only, no guard built → substantiated DELIVER).
  Prescription-as-guard form: delivers invariant + steering-vector + self-test, not prose. **Delivery on Operator
  trigger** (Frontier files; not covered by wu-wei DM standing-permission, which is responses-only).
### Done
