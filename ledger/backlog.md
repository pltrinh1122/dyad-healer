...
### Open
- [ ] **CI lexical-guard false-positive cascade — DRAFTED + RATIFIED, pending filing** 2026-05-29. From case-04 S9 carry ("guard bug + harmonization deferred"). **One filing, one violation, ailment-only** (falsify pass collapsed 2→1): **V1** CI-guard precision/scope — `tests/test_lexical_guard.py` `check_content()` raw-substring match + scans tracked `.venv` (670 files, not in `immune_zones`), **diverging from production drivers** (`drivers/audit_daemon.py:287`, `drivers/knowledge_accrual_skill.py:169`) which already use `\b` boundaries + scan `kb/` only. Harmonized SG-0003/SG-0002/NS-0001. **Dropped at falsify:** V2 deferred-debt locality — stale; `proposed:`/`known_debt` is the patient's *intentional* v3 deferral ("resolved when gravitational pull demands it"); guard ignoring it is correct. A (`#1393`-V2 path) dropped — already filed. Re-verified live at `dz-cil@8507664`. → `recommendations/2026-05-29-lexical-guard-false-positive-cascade.md`. **Next:** Healer files via Requirement Intake template on Operator trigger.
### Done
