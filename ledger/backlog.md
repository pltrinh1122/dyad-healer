...
### Open
- [ ] **Authority/posture codification** — kb-codify the 2026-05-30 dialectic: (1) *authority tracks incident-grounding* (prescription post-incidence/action-authority vs research pre-incidence/attention-authority); (2) *deliver prescriptions / post research* (wu-wei verb-matching); (3) *uncertainty-is-law / scientific method* (patient self-heal = null hypothesis; intervention requires evidence to reject it; only-arguable → don't intervene). Plus the **named invariant** `self-detection = G1∧G2` (RATIFIED 2026-05-30) — theory earns authority empirically (retrodiction 4/4 + forward experiment P1/P2). Memories written (`uncertainty-is-law`, `deliver-prescription-post-research`). **Plan PROPOSED 2026-05-31 (awaiting ratification):** two homes — (A) principles 1/2/3 → new `governance-map.md` §"Intervention authority & posture" (operational layer of Charter #5/#6; extends posture-gated-authority with the intervention-threshold + force-verb axis); (B) `G1∧G2` invariant → new cross-cutting principle in `seizure-taxonomy.md` (detector/observer law; seeds the queued detectability axis). **Open fork at hand-off:** fold A into governance-map (Healer recommendation, wu-wei) vs standalone `kb/intervention-doctrine.md`. Cycle-bound: glossary-check + anchor-sync + form-alignment. Operator ratifies. → ledger "2026-05-31 Codification Plan".
