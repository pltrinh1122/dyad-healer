...
- [ ] ⏸ **Practice governance discipline** — codify two anchor-maintenance rules: **(a) anchor-sync** — every ratified new `kb/` doc updates `CLAUDE.md`'s Durable practice map in the same ratification cycle (closes the anchor-divergence gap); **(b) Practice lifecycle** — archival convention (`ledger/archive/YYYY-QN.md` for sections > 2 quarters old; `ledger/backlog-archive.md` for Done items > 1 quarter old) + new `kb/practice-snapshot.md` (RATIFIED-state digest, refreshed each ratification cycle) + CLAUDE.md anchor points to snapshot + active loop, not full historical ledger. Trigger criteria (falsifiable): act when ledger.md > 1000 lines OR backlog.md Done > 50 items OR verify-before-asserting canary trips on practice-loading. **Queued post-Loop-#4.** Provenance: 2026-05-28 two [FEEDBACK] falsifications — (a) "Loop #4 changes won't survive /restart without CLAUDE.md capture" (literal claim falsified for committed changes; surviving +1 = the new-doc anchor-sync rule); (b) "ledger/backlog growth will overwhelm context with uncontrolled degradation" (literal "overwhelmed" falsified at current scale ~25k tokens / 1M context; surviving +1 = lifecycle disciplines before unbounded growth intersects the long-context degradation curve; clock measured in months at current ~10%/session growth rate).
### Done
