...
### Open
- [ ] **Authority/posture codification** — kb-codify the 2026-05-30 dialectic: (1) *authority tracks incident-grounding* (prescription post-incidence/action-authority vs research pre-incidence/attention-authority); (2) *deliver prescriptions / post research* (wu-wei verb-matching); (3) *faith-in-the-uncertainty-zone* (only-arguable blind spot → trust the patient's immune system, don't intervene). Memories written (`faith-in-uncertainty-zone`, `deliver-prescription-post-research`); kb home TBD (charter/glossary touch). Operator ratifies. → ledger "2026-05-30 Self-detectability gradient".
